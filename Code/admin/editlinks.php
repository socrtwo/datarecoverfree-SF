...
if($checker == 'tester'){
	...
	$insert_sql = mysql_query("UPDATE `links` SET `name` = '$name', `link` = '$link' WHERE `id` ='$eid'");
	
	if($insert_sql == '1')
		header('Location: links.php?info=Link Updated Successfully');
}else{
	...
	if($id == ''){
		...
		$select_sql = mysql_query("select * from links where id='$id'");
		//print_r($select_sql);
...
<title>Edit Details - Custom Links</title>
...
<form action="editlinks.php" name="edit_links" method="post" onsubmit="return validate1();">
...
<div class="fieldholder"><div class="fleft"><span>Link :</span></div><div class="fright"><input type="text" name="link1" id="link1" value="<?php echo $res_sql['link']; ?>" /></div></div>
<div class="fieldholder"><div class="fleft"><span>&nbsp;</span></div><div class="fright"><input type="submit" value="Update" /></div></div>
</form>
<script type="text/javascript">
function validate1()
{
	var frm = document.edit_links;
