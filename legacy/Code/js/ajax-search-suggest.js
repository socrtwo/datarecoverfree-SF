...
$(document).ready(function(){ //when the browser has rendered the page...
	...
	function updateResults(latestQuery){
		if(latestQuery.length > 0){ //if there is a query to process...
			$.post("http://localhost/projects/s2services/auto-suggest.php", {latestQuery: latestQuery}, function(data){ //..send that query to the php script "auto-suggest.php" via ajax
