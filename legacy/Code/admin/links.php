<?php define('TITLE','Custom Links Page'); require('includes/header.php'); ?>
<?php
	$del_id = isset($_REQUEST['del_id'])?mysql_real_escape_string($_REQUEST['del_id']):'';
	
	if($del_id){
		$del_sql = mysql_query("delete from links where id='$del_id'");
		if($del_sql == 'true'){
			header('Location: links.php?info=Link Deleted Successfully');
		}else
			header('Location: links.php?info=Error in Deleting. Please try again later');
	}
?>
...
<?php	/*
	...
	*/
	...
	$targetpage = "links.php"; 	//your file name  (the name of this file)
...
<div id="admintablewrapper">
  <table width="100%" bgcolor="#FFFFFF" cellpadding="3" cellspacing="3" border="0" style="font:'trebuchet MS'; font-size:12px; margin:0px;">
	...
	<?php
        ...
        <td class="actions"><a href="editlinks.php?id=<?php echo $row['id']; ?>">Edit</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:void(0);" onClick='var answer = confirm("Are you sure want to delete this?"); if (answer){ window.location = "links.php?del_id=<?php echo $row['id']; ?>"; }'>Delete</a></td>
	    </tr>
