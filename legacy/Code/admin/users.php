<?php define('TITLE','Registered Users Page'); require('includes/header.php'); ?>
<?php
	...
	if($del_id){
		...
		if($del_sql == 'true'){
			header('Location: users.php?info=Users Deleted Successfully');
		}else
			header('Location: users.php?info=Error in Deleting. Please try again later');
	}
	
	if($update_id){
		$update_sql = mysql_query("select * from `users` where `id`='$update_id'");
		if(mysql_num_rows($update_sql)>0){
			$update_res = mysql_fetch_array($update_sql);
			if($update_res['status']=='1'){
				$update_sql1 = "update `users` set `status`='0' where `id`='$update_id'";
			}else{
				$update_sql1 = "update `users` set `status`='1' where `id`='$update_id'";				
			}
			$update_res1 = mysql_query($update_sql1);
			if($update_res1){
				header('Location: users.php?info=Status Updated Successfully');
			}else{
				header('Location: users.php?info=Error in updation. Try again later');
			}
		}
	}
?>
...
<div id="admintablewrapper">
  <table width="100%" bgcolor="#FFFFFF" cellpadding="3" cellspacing="3" border="0" style="font:'trebuchet MS'; font-size:12px; margin:0px;">
	...
	?>
        ...
        <td class="actions"><a href="users.php?update_id=<?php echo $row['id']; ?>">Update Status</a>  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <a href="javascript:void(0);" onClick='var answer = confirm("Are you sure want to delete this?"); if (answer){ window.location = "users.php?del_id=<?php echo $row['id']; ?>"; }'>Delete</a></td>
	    </tr>
